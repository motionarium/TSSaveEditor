...
namespace TruckLib.ScsMap
{
    ...
    public class Map : IItemContainer
    {
        ...
        /// <summary>
        /// When false, skips the final cross-reference pass (cheaper for UID-only scans).
        /// </summary>
        protected virtual bool ShouldUpdateReferences => true;
        ...
        private void ReadSectors(string mapDirectory, IFileSystem fs, IList<SectorCoordinate> sectors = null)
        {
            ...
            foreach (var (_, sector) in Sectors)
            {
                ...
                {
                    // Skip broken / unsupported sectors so Road/Prefab UIDs from
                    // the rest of the map can still be collected (save editors).
                    Trace.WriteLine($"Skipping sector {sector}: {ex.Message}");
                    OnSectorLoadFailed(sector, ex);
                }
                i++;
            }
        }
        ...
        /// <summary>
        /// Called when a sector fails to load (unsupported item type, corrupt data, etc.).
        /// Default: no-op; map loading continues with remaining sectors.
        /// </summary>
        protected virtual void OnSectorLoadFailed(Sector sector, Exception exception) { }

        /// <summary>
        /// Called after a sector has been loaded successfully.
        /// </summary>
        protected virtual void OnSectorLoaded(Sector sector) { }
        ...
        private void ReadBase(string path, IFileSystem fs)
        {
            ...
            var header = new Header
            {
                // hex1 map ships as 906; current TruckLib targets 907.
                EnforceVersion = Header.EnforceVersionBehavior.AllowLower
            };
            header.Deserialize(r);
